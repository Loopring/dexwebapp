...
export default class Wallet {
   ...
   * @param sendByMetaMask
   */
  async forceWithdrawal(payload, sendByMetaMask) {
    payload['from'] = this.address;
    console.log('payload', payload);
    const tx = exchange.forceWithdraw(payload);
    const response = sendByMetaMask
      ? await sendTransaction(this.web3, tx)
      : await signEthereumTx(this.web3, this.address, tx);
    return response['result'];
  ...
  async signOffchainWithdrawWithDataStructure(data) {
    ...
    const eddsaSig = exchange.signOffChainWithdraw(data, this.keyPair);
  ...
  async isContractWallet() {
    return await isContract(this.web3, this.address);
  }
}
