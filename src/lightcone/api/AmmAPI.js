...
export async function getAmmMarkets() {
  ...
  const ammMarkets = response['data'];
  return ammMarkets.map((market) => {
    return {
      ...market,
      isSwapEnable: isSwapEnable(market.status),
      isJoinEnable: isJoinEnable(market.status),
      isExitEnable: isExitEnable(market.status),
      isShowMarket: isShowMarket(market.status),
    };
  });
}
